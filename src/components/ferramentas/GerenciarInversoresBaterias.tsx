...
import { Plus, Edit2, Power, PowerOff, X, Save, Trash2, Upload, Zap, Battery } from 'lucide-react';
import {
...
export default function GerenciarInversoresBaterias({ onClose, onSalvo, abaInicial = 'inversores' }: {
  ...
  const carregar = async () => {
    setLoading(true);
    try {
      const [inv, bat] = await Promise.all([getInversoresHibridoDB(false), getBateriasHibridoDB(false)]);
      setInversores(inv);
      setBaterias(bat);
    } catch (e: any) {
  ...
  const fechandoForm = () => { setFormInv(null); setFormBat(null); };
      ...
      <div className="bg-card rounded-xl max-w-3xl w-full max-h-[85vh] overflow-y-auto p-6 space-y-4" onClick={e => e.stopPropagation()}>
        ...
        {!formInv && !formBat && (
          <p className="text-xs text-muted-foreground -mt-2">
            Placas são gerenciadas em Admin → Equipamentos → Placas.
          </p>
        )}

        {/* ── LISTA / NOVO — INVERSORES ── */}
        {aba === 'inversores' && !formInv && !formBat && (
          <>
